...
-- 3. Checking the average salary

select round(avg(salary), 2) as average_salary from Employees;

-- 4. Counting active employees

Select count(*) as active_employees_amount from Employees where ative = 1;

-- 5. Counting employees in each department

select department, count(*) employees_number from Employees group by department;

-- 6. Checking the average age of the active employees
select avg(age) as average_age from Employees where active = 1;

-- 7. Checking the average salary of the employees in marketing

select avg(salary) as average_salary from Employees
where department = "Marketing";

-- 8. Checking greatest and lowest salary of employees in "Soporte"
...
select count(*) as employees_older_than_30 from Employees
where age > 30;

-- 10.Cheking average age of no active employees

SELECT AVG(age) AS average_age FROM Employees WHERE active = 0;
...
SELECT name FROM Employee WHERE age = (SELECT MAX(age) FROM Employees);
